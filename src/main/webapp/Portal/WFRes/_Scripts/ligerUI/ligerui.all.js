...
//同步方式获取本地化字符串
ligeruiGlobalString = $.Lang("ligeruiGlobalString");
//$.ajax({
//    type: "GET",
//    url: _PORTALROOT_GLOBAL + "/Ajax/GlobalHandler.ashx?Code=LigerGridString_errorMessage,LigerGridString_pageStatMessage,LigerGridString_loadingMessage,LigerGridString_findTextMessage,LigerGridString_noRecordMessage,LigerGridString_isContinueByDataChanged,LigerGridString_cancelMessage,LigerGridString_saveMessage,LigerGridString_applyMessage,LigerGridString_draggingMessage,LigerDialogString_titleMessage,LigerDialogString_ok,LigerDialogString_yes,LigerDialogString_no,LigerDialogString_cancel,LigerDialogString_waittingMessage",
//    async: false,
//    dataType: "json",
//    success: function (data) {
//        if (data.IsSuccess) {
//            ligeruiGlobalString = data.TextObj;
//        }
//    }
//});
...
(function ($) {
    ...
    $.ligerui.controls.ComboBox.ligerExtend($.ligerui.controls.Input, {
        ...
        _setResize: function (resize) {
            ...
            if (resize && $.fn.ligerResizable) {
                g.selectBox.ligerResizable({
                    handles: 'se,s,e', onStartResize: function () {
                        g.resizing = true;
        ...
        setTree: function (tree) {
            ...
            if (tree.checkbox != false) {
                ...
                tree.onSelect = function (node) {
                    ...
                    //modified by CHC 
                    //异步加载的时候，子节点可能还没加载出来，但是不一定是叶子节点
                    //if (p.treeLeafOnly && node.data.children) return;
                    if (p.treeLeafOnly) {
                        if ($.isFunction(p.tree.isLeaf) && !p.tree.isLeaf(node.data)) {
                            return;
                        }
                    }
            ...
            if (!$.isFunction(tree.onAfterAppend)) {
        ...
        updateStyle: function () {
            var g = this, p = this.options;
            //修改 下拉的默认值 无效bug， eadit by chc
            //p.initValue = g._getValue();
            p.initValue = p.initValue || g._getValue();
            g._dataInit();
        },
        _dataInit: function () {
            var g = this, p = this.options;
    ...
    $.ligerui.controls.DateEditor.ligerExtend($.ligerui.controls.Input, {
        ...
        _render: function () {
            ...
            $(this.element).focus(function () {
                WdatePicker({ dateFmt: 'yyyy-MM-dd', minDate: '2010-01-01 00:00:00', maxDate: '2050-12-31 23:59:59' });
            });
            g.text = $(this.element);
            g.inputText = $(this.element);
        },
...
(function ($) {
    $.ligerDefaults.DialogString = {
        //titleMessage: '提示1',                     //提示文本标题
        //ok: '确定1',
        //yes: '是1',
        //no: '否1',
        //cancel: '取消1',
        //waittingMessage: '正在等待中,请稍候...1'
        titleMessage: ligeruiGlobalString.LigerDialogString_titleMessage,                     //提示文本标题
        ok: ligeruiGlobalString.LigerDialogString_ok,
        yes: ligeruiGlobalString.LigerDialogString_yes,
        no: ligeruiGlobalString.LigerDialogString_no,
        cancel: ligeruiGlobalString.LigerDialogString_cancel,
        waittingMessage: ligeruiGlobalString.LigerDialogString_waittingMessage
    };
    ...
    l.controls.Dialog.ligerExtend(l.core.Win, {
        ...
        _render: function () {
            ...
            //提示内容 没有自动换行  edit by chc
            g.dialog.content.css("word-wrap", "break-word");
            ...
            if (!p.fixedType) {
                ...
                else p.top = top = 0.5 * ($(window).height() - g.dialog.height()) + $(window).scrollTop();
        ...
        doMax: function (slide) {
            ...
            //add by luwei : 父窗口下滑，打开子对话框，最大化对话框上移BUG
            g.dialog.css({position:'fixed'});
            if (slide) {
        ...
        hideTitle: function () {
            var g = this;
            //$("tr:first", g.dialog).hide();
            $("div[class='l-dialog-tc-inner']", g.dialog).hide();
            $(".l-dialog-tc", g.dialog).css("height", "6px");
            $(".l-dialog-tl", g.dialog).css("height", "6px");
            $(".l-dialog-tr", g.dialog).css("height", "6px");
        },
        ...
        doClose: function () {
            ...
            //Bug修改：多次打开Dialog后就有问题，得关闭时移除  by CHC
            l.remove(this);
        },
...
(function ($) {
    ...
    $.ligerDefaults.Grid = {
        ...
        treeAsync: false,                        //表格树异步加载
        treeUrl: null                            //表格树异步加载url
    };
    ...
    $.ligerDefaults.GridString = {
        ...
        errorMessage: ligeruiGlobalString.LigerGridString_errorMessage,
        pageStatMessage: ligeruiGlobalString.LigerGridString_pageStatMessage,
        pageTextMessage: 'Page',
        loadingMessage: ligeruiGlobalString.LigerGridString_loadingMessage,
        findTextMessage: ligeruiGlobalString.LigerGridString_findTextMessage,
        noRecordMessage: ligeruiGlobalString.LigerGridString_noRecordMessage,
        isContinueByDataChanged: ligeruiGlobalString.LigerGridString_isContinueByDataChanged,
        cancelMessage: ligeruiGlobalString.LigerGridString_cancelMessage,
        saveMessage: ligeruiGlobalString.LigerGridString_saveMessage,
        applyMessage: ligeruiGlobalString.LigerGridString_applyMessage,
    ...
    $.ligerui.controls.Grid.ligerExtend($.ligerui.core.UIComponent, {
        ...
        _init: function () {
            ...
            if (p.tree)//启用分页模式
            {
                ...
                p.tree.isParent = p.tree.isParent || function (rowData) {
                    ...
                    //如果孩子树是null 的话，不显示展开按钮 by chc
                    if (exist) {
                        exist = rowData[p.tree.childrenName] != null;
                    }
                    return exist;
                };
        ...
        _render: function () {
            ...
            //chc
            gridhtmlarr.push("          <div class='l-bar-group l-bar-control'>");
        ...
        _calculateGridBodyHeight: function (h) {
            ...
            //Edit by chc,值变字符串拼接
            var gridHeaderHeight = Number(p.headerRowHeight) * (g._columnMaxLevel - 1) + Number(p.headerRowHeight) - 1;
            h -= gridHeaderHeight;
            return h;
        },
        ...
        */
        ...
        changePageToFirst: function (ctype) {
            var g = this, p = this.options;
            if (g.loading) return true;
            if (p.dataAction != "local" && g.isDataChanged && !confirm(p.isContinueByDataChanged))
                return false;
        ...
        toggleCol: function (columnparm, visible, toggleByPopup) {
            ...
            if (column.frozen) {
                ...
                $("div:first", g.gridheader).width("100%");
                $("div:first", g.gridbody).width("100%");
            }
        ...
        setColumnWidth: function (columnparm, newwidth) {
            ...
            //内容不能填满整个单元格 edit by chc
            //$(cells).css('width', newwidth).find("> div.l-grid-row-cell-inner:first").css('width', newwidth - 8);
            $(cells).css('width', newwidth).find("> div.l-grid-row-cell-inner:first").css('width', "100%");
        ...
        toggle: function (rowParm) {
            ...
            //异步加载数据并添加到节点树中,尚未判断是否已经异步加载过
            //只有当当前样式为close时才异步加载，且只加载一次
            var flag = linkbtn.hasClass('l-grid-tree-link-close');
            if (p.treeAsync && flag && window[rowdata.ObjectID + '_load'] == null) {
                $.ajax({
                    url: p.treeUrl + "&id=" + rowdata.ObjectID,
                    type: "POST",
                    async: false,
                    ...
                    success: function (data) {
                        ...
                        //添加行
                        //已经加载过一次
                        window[rowdata.ObjectID + '_load'] = true;
                        var datas = [];
                        for (var i = 0; i < obj.length; i++) {
                            var data;
                            if (obj[i].Type == 0)
                                data = { DisplayName: obj[i].DisplayName, ObjectID: obj[i].ObjectID, Code: obj[i].Code, Type: obj[i].Type, Version: obj[i].Version, PublishedTime: obj[i].PublishedTime, Frequent: obj[i].Frequent, children: [] };
                            else
                                data = { DisplayName: obj[i].DisplayName, ObjectID: obj[i].ObjectID, Code: obj[i].Code, Type: obj[i].Type, Version: obj[i].Version, PublishedTime: obj[i].PublishedTime, Frequent: obj[i].Frequent };
                            datas.push(data);
                        }
                        g.addRows(datas, null, true, rowdata);
                    },
                    error: function (XMLHttpRequest, textStatus, errorThrown) {
                        alert(textStatus);
                    }
            ...
            for (var i = 0, l = children.length; i < l; i++) {
                ...
                if (opening) {
                    if (o.__hasChildren && p.treeAsync && o.children.length == 0) {
                        $(".l-grid-tree-link", currentRow).removeClass("l-grid-tree-link-open").addClass("l-grid-tree-link-clode");
                        currentRow.show();
                    }
                    else {
        ...
        _initBuildGridHeader: function () {
            ...
            for (var level = 1; level <= g._columnMaxLevel; level++) {
                ...
                //表头添加多一个列，为了适应表格内容出现滚动条时，错位的情况 edit by chc
                tr.append($("<td style='width:8px;'></td>"));
            }
        ...
        _fillGridBody: function (data, frozen) {
            ...
            //设置l-grid-body-inner 的宽度为100%,不让出现滚动条 by chc
            //$("> div:first", g.gridbody).width(g.gridtablewidth);
            $("> div:first", g.gridbody).width("100%");
            g._onResize();
        },
        _showData: function () {
            ...
            if (!data || !data.length) {
                ...
                //设置 l-grid-body-inner 高宽为100%
                $("<div></div>").addClass("l-grid-body-inner").appendTo(g.gridbody).css({
                    width: "100%",//g.gridheader.find(">div:first").width(),
                    height: "100%"//g.gridbody.height()
                });
        ...
        _getTreeCellHtml: function (oldContent, rowdata) {
            ...
            //异步加载树时先闭合不展开
            if (isExtend && isParent && p.treeAsync && window[rowdata.ObjectID + '_load'] == null) {
                content += "<div class='l-grid-tree-space l-grid-tree-link l-grid-tree-link-close'></div>";
            }
            else if (isExtend && isParent) {
            ...
            //计算内容长度是不是超过，超过的时候，显示...
            //var showContent = oldContent;
            //var tempObj = $("<pre>").hide().html(showContent).appendTo(document.body);
            //var tempWidth = tempObj.width();
            //var colWidth = g._getColumnWidth(rowdata);
            //while (tempWidth > colWidth) {
            //    showContent = showContent.substring(0, showContent.length - 6) + "...";
            //    tempObj.html(showContent);
            //     tempWidth = tempObj.width();
            //}
            //tempObj.remove();
            //content += "<span class='l-grid-tree-content' title='" + oldContent + "'>" + showContent + "</span>";
            ...
            if ($.fn.ligerDrag && p.colDraggable) {
                ...
                g.gridheader.add(g.f.gridheader).ligerDrag({
                    ...
                    onStartDrag: function (current, e) {
                        //add by chc
                        if ($.isFunction(p.onColStartDrag)) {
                            return p.onColStartDrag.call(this, g, p, current, e);
                        }
                    ...
                    onDrag: function (current, e) {
                        //add by chc
                        if ($.isFunction(p.onColDragging)) {
                            if (!p.onColDragging.call(this, g, p, current, e))
                                return;
                        }
                    ...
                    onStopDrag: function (current, e) {
                        //add by chc
                        if ($.isFunction(p.onColStopDrag)) {
                            if (!p.onColStopDrag.call(this, g, p, current, e))
                                return;
                        }
        ...
        _onClick: function (e) {
            ...
            else if (src.row && g.enabledCheckbox()) //复选框选择行
            {
                ...
                if (src.checkbox || !selectRowButtonOnly) {
                    ...
                    //取消全选状态
                    var hrow = $("#"+g.id).find(".l-grid-hd-row");
                    if(uncheck){
                    	hrow.removeClass("l-checked");
                    }else{
                    	//全部选择时自动勾选全选
                    	if(g.selected.length == g.rows.length){
                    		hrow.addClass("l-checked");
                    	}
                    }
    ...
    $.ligerui.controls.Layout.ligerExtend($.ligerui.core.UIComponent, {
        ...
        _onResize: function () {
            ...
            if (g.left) {
                ...
                /***** 
                设置左侧可以布局有滚动栏  Add BY CHC
                *****/
                g.left.content.height(g.left.height() - g.left.header.height() - 3);
                g.left.content.css("overflow", "auto");
                g.left.content.css("overflow-y", "auto");
            }
            if (g.right) {
...
(function ($) {
    ...
    $.ligerui.controls.Menu.ligerExtend($.ligerui.core.UIComponent, {
        ...
        _render: function () {
            ...
            g.menu = g.createMenu();
            g.element = g.menu[0];
            g.menu.css({ top: p.top, left: p.left, width: $.ligerDefaults.Menu.width });
...
(function ($) {
    ...
    $.ligerDefaults.TabString = {
        closeMessage: $.Languages.msgGlobalString.CloseCurrentPage,
        closeOtherMessage:  $.Languages.msgGlobalString.CloseOtherPage,
        closeAllMessage:  $.Languages.msgGlobalString.CloseAllPage,
        reloadMessage: $.Languages.msgGlobalString.RefreshPage
    };
    ...
    $.ligerui.controls.Tab.ligerExtend($.ligerui.core.UIComponent, {
        ...
        _render: function () {
            ...
            if (p.contextmenu && $.ligerMenu) {
                g.tab.menu = $.ligerMenu({
                    width: 100, items: [
                        {
                            text: p.closeMessage, id: 'close', icon: "candle", click: function () {
                                g._menuItemClick.apply(g, arguments);
                            }
                        },
                        {
                            text: p.closeOtherMessage, id: 'closeother', icon: "customers", click: function () {
                                g._menuItemClick.apply(g, arguments);
                            }
                        },
                        {
                            text: p.closeAllMessage, id: 'closeall', icon: "export", click: function () {
                                g._menuItemClick.apply(g, arguments);
                            }
                        },
                        {
                            text: p.reloadMessage, id: 'reload', icon: "refresh", click: function () {
                                g._menuItemClick.apply(g, arguments);
                            }
                        }
                    ]
        ...
        //获取标题 add by chc
        getHeader: function (tabid) {
            return $("li[tabid=" + tabid + "] a", this.tab.links.ul).text();
        },
        ...
        overrideTabItem: function (targettabid, options) {
            ...
            //改为如果不存在就返回，因为覆盖肯定是覆盖已经存在的 edit by chc
            //如果已经存在
        ...
        setHeader: function (tabid, header) {
            var tagIndex = header.lastIndexOf("["),
                name = header,
                tag = "",
                tabName = "",
                tab = $("li[tabid=" + tabid + "] a", this.tab.links.ul);

            if (tagIndex !== -1) {
                name = header.substring(0, tagIndex);
                tag = header.substring(tagIndex);
            }
            tabName = (name.length > 12 ? name.substring(0, 12) + "..." : name) + tag;
            tab.text(tabName).attr("title", header);
        },
        //选中tab项
        ...
        addTabItem: function (options) {
            ...
            //如果超过最大的tab数量，则不再添加tab 页 edit by chc
            //if (options.MaxTabCount != null) {
            //    if (g.getTabItemCount() >= options.MaxTabCount) {
            //        $.H3Dialog.Warn({ content: "已超过最大的tab页个数" });
            //        return;
            //    }
            //}
            ...
            // tab的文本过长时，截取显示
            if (text.length > 12) {
                $("a", tabitem).text(text.substring(0, 12) + "...");
                $("a", tabitem).attr("title", text);
            }
            else {
                $("a", tabitem).text(text);
            }
...
(function ($) {
    ...
    $.ligerui.controls.TextBox.ligerExtend($.ligerui.controls.Input, {
        ...
        checkValue: function () {
            var g = this, p = this.options;
            //add by chc 添加校验方法
            if ($.isFunction(p.checkValue)) {
                p.checkValue.call(this);
                return;
            }
...
(function ($) {
    ...
    $.ligerDefaults.Tree = {
        ...
        onError: function () { },
        onClick: function () { },
        //添加鼠标移动上去的事件  add By CHC
        onMouseOver: function () { },
        //添加鼠标移动出去的事件  add By CHC
        onMouseOut: function () { },
        //节点开始拖拽的事件   add By CHC
        onStartDrag: null,
        //节点拖动中事件    add By CHC
        onDragging: null,
        //节点拖动结束事件  add By CHC
    ...
    $.ligerui.controls.Tree.ligerExtend($.ligerui.core.UIComponent, {
        ...
        loadData: function (node, url, param, e) {
            ...
            $.ajax({
                ...
                success: function (data) {
                    /* 修改隐藏loading设置，返回null也要隐藏， eadit by chc */
                    e.hideLoading();
                    if (!data) return;
                    g.append(node, data);
                    g.trigger('success', [data]);
        ...
        _getTreeHTMLByData: function (data, outlineLevel, isLast, isExpand) {
            ...
            for (var i = 0; i < data.length; i++) {
                ...
                for (var j = 0; j < g.sysAttribute.length; j++) {
                    //jquery 1.10 版本下，ie7 下会报错；对应元素不能包含的属性会报错，这里加判断 by chc
                    if (this.element.tagName.toLowerCase() != "a"
                        && g.sysAttribute[j] == "href")
                        continue;
                ...
                if (g.hasChildren(o)) {
                    ...
                    if (p.parentIcon) {
                        ...
                        treehtmlarr.push('">');
                        if (p.iconFieldName && o[p.iconFieldName])
                            if (o[p.iconFieldName].indexOf("png") < 0 && o[p.iconFieldName].indexOf("gif") < 0
                                && o[p.iconFieldName].indexOf("jpg") < 0) {
                                treehtmlarr.push('<i class="' + o[p.iconFieldName] + '" />');
                            }
                            else { treehtmlarr.push('<img src="' + o[p.iconFieldName] + '" />'); }
                ...
                if (g.hasChildren(o)) {
                    ...
                    if (delay) {
                    	...
                    	//update by luwei : skip if o has no children
                    	if(o.children) {
        ...
        _setTreeEven: function () {
            ...
            if (g.hasBind("mouseover")) {
                g.tree.bind("mouseover", function (e) {
                    ...
                    if (obj.tagName.toLowerCase() == "a" || obj.tagName.toLowerCase() == "span" || $(obj).hasClass("l-box")) {
                        ...
                        treeitem = $(obj);
                        treeitemBody = $(obj).children("div.l-body");
                    }
                    if (!treeitem) return;
                    var treedataindex = parseInt(treeitem.attr("treedataindex"));
                    var treenodedata = g._getDataNodeByTreeDataIndex(g.data, treedataindex);
                    return g.trigger('mouseover', [{ data: treenodedata, target: treeitem[0], targetItem: treeitemBody[0] }, e]);
                });
            }
            ...
            if (g.hasBind("mouseout")) {
                g.tree.bind("mouseout", function (e) {
                    ...
                    var treedataindex = parseInt(treeitem.attr("treedataindex"));
                    var treenodedata = g._getDataNodeByTreeDataIndex(g.data, treedataindex);
                    return g.trigger('mouseout', [{ data: treenodedata, target: treeitem[0], targetItem: treeitemBody[0] }, e]);
                });
            }
            ...
            g.tree.click(function (e) {
                ...
                if ($(obj).hasClass("l-checkbox")) {
                    ...
                    var iconCss = treeitembtn.parent("div").find("i").attr("class");
                    if (iconCss != undefined && iconCss.indexOf("fa-folder") > -1) {
                        treeitembtn.parent("div").find("i").attr("class", "fa fa-folder-o")
                    }
                    if (p.slide)
                    ...
                    var iconCss = treeitembtn.parent("div").find("i").attr("class");
                    if (iconCss != undefined && iconCss.indexOf("fa-folder") > -1) {
                        treeitembtn.parent("div").find("i").attr("class", "fa fa-folder-open-o")
                    }
            ...
            if ($.fn.ligerDrag && p.nodeDraggable) {
                ...
                g.tree.ligerDrag({
                    ...
                    onStartDrag: function (current, e) {
                        ...
                        //add by chc
                        if ($.isFunction(p.onStartDrag)) {
                            if (!p.onStartDrag.call(this, g, p, current, e))
                                return false;
                        }
                    ...
                    onDrag: function (current, e) {
                        //add by chc
                        if ($.isFunction(p.onDragging)) {
                            if (!p.onDragging.call(this, g, p, current, e))
                                return;
                        }
                        ...
                        for (var i = 0, l = g.nodes.length; i < l; i++) {
                            var nd = g.nodes[i];
                            if (nd.__status == "delete") continue;
                    ...
                    onStopDrag: function (current, e) {
                        //add by chc
                        if ($.isFunction(p.onStopDrag)) {
                            if (!p.onStopDrag.call(this, g, p, current, e))
                                return;
                        }
